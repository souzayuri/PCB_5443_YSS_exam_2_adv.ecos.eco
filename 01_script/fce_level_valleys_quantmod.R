...
fce<-read.csv("fce.tsph6+100.level.csv")
summary(fce)
...
threshold.valley<-10 #Use for SRS
threshold.valley<-5 #Use for TS/Ph

# STEP 3: Function to identify peaks[valleys] --------
fcevalleys<-findValleys(fcexts, thresh=threshold.valley)
plot(fcexts[fcevalleys-1])

#contrast this result against a 5 cm rain deviation threshold
threshold.5<-5 
fcevalleys.5<-findValleys(fcexts, thresh=threshold.5)
plot(fcexts[fcevalleys.5-1])
...
valleys<-as.data.frame(fcexts[fcevalleys-1])

#plot peaks onto raw data for precipitation
valleys_graphic<-ggplot(fce, aes(x = Date, y = TS.Ph7)) +
  geom_line(colour='blue') +
  labs(x = "Date",
       y = "Water Level + 100 (cm)")+
  geom_point(data=valleys,aes(x = as.POSIXct(row.names(valleys)), y = V1), 
             colour='red', size=4)
valleys_graphic
#save graphic - revise file name for your dataset
ggsave("fce_tsph7_level_valleys.jpg",valleys_graphic,dpi=300)
...
# STEP 5: Calculate metrics of valleys ----------
#how many peaks per total obs, which are in units of days (here, obs = 365 days*33 years) ?
valley_per_d<-length(valleys$V1)/length(fce$TS.Ph7)
valley_per_d

#how many valleys per year?
valley_per_y<-length(valleys$V1)/nyears(fcexts)
valley_per_y

#average valley magnitude (in cm for water level)
valley_mean<-mean(valleys$V1)
valley_mean

#peak standard deviation
valley_sd<-sd(valleys$V1)

#peak CV
valley_CV<-sd(valleys$V1)/mean(valleys$V1)
valley_CV


# STEP 6: Standardized regression models for temporal change--------
#valley number vs. time: Has the number of valleys increased/decreased over time?
# get slope of (number of valleys per year for each year) vs. year (and p-value) to look for temporal change in number of valleys

# add year and time columns to valleys dataset
valleys$time<-as.POSIXct(row.names(valleys))
valleys$year<-as.numeric(format(as.POSIXct(row.names(valleys)),"%Y"))
summary(valleys)

# first, add any missing years that had no valleys (add zeros) - probably a more efficient way to do this...
year<-min(as.numeric(format(as.POSIXct(fce$Date),"%Y"))):max(as.numeric(format(as.POSIXct(fce$Date),"%Y")))
years<-as.data.frame(year)
years
valley.sum<-valleys %>% group_by(year) %>% summarise(mean.valley=mean(V1), count=n())
valley.sum
valley.number<-merge(years,valley.sum,by.x="year",by.y="year",all.x=TRUE)
valley.number[is.na(valley.number)] <- 0 
valley.number

# second, build the stats models and save the slope and p as output
valley.number.lm<-lm(scale(count)~scale(year),data=valley.number)
...
#plot(valley.number.lm) # turn this on to check model statistical assumptions
lmsum.number<-summary(valley.number.lm)
valley.number.slope<-valley.number.lm$coefficients[2]
valley.number.slope
valley.number.p<-lmsum.number$coefficients[2,4]
valley.number.p

valley.number.graph<-ggplot(data=valley.number, aes(x = year, y = count)) + 
  geom_point(color='darkslategrey',size=4) +
  geom_smooth(method = "lm",  se = FALSE, color="darkslategrey")+
  ylab("Number of valleys")+
  xlab("Year")+
  theme_tufte(base_size = 14, base_family="sans")
valley.number.graph
#save graphic - revise file name for your dataset
ggsave("tsph7_level_valley.number.jpg",valley.number.graph,dpi=300,width=10, height=4)

# valley magnitude (all valleys) vs. time: Has the magnitude of valleys increased/decreased over time?
# get slope of magnitude of valleys vs. time (and p-value)
valley.magnitude.lm<-lm(scale(V1)~scale(time),data=valleys)

# plot(valley.magnitude.lm) # turn this on to check model statistical assumptions
lmsum.mag<-summary(valley.magnitude.lm)
valley.magnitude.slope<-valley.magnitude.lm$coefficients[2]
valley.magnitude.slope
valley.magnitude.p<-lmsum.mag$coefficients[2,4]
valley.magnitude.p

valley.mag.graph<-ggplot(data=valleys, aes(x = time, y = V1)) + 
  geom_point(color='darkslategrey',size=4) +
  geom_smooth(method = "lm",  se = FALSE, color="darkslategrey")+
  ylab("Average valley magnitude (mm)")+
  xlab("Year")+
  theme_tufte(base_size = 14, base_family="sans")
valley.mag.graph
#save graphic - revise file name for your dataset
ggsave("tsph7_level_valley.magnitude.jpg",valley.mag.graph,dpi=300,width=10, height=4)

# STEP 7:  Save metrics into your site location data frame ------------
pulse_metrics_fce<-data.frame(lter,site,driver,units,pv,nyears,nmonths,valley_mean,valley_sd,valley_CV,valley_per_y,valley_per_d,
                                valley.number.slope,valley.number.p,valley.magnitude.slope,valley.magnitude.p,threshold.valley)
